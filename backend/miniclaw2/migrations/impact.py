...
def layout_recovery_guidance(root: Path) -> list[dict[str, Any]]:
    ...
    for journal_path in sorted((root / ".migration-local/transactions").glob("*/journal.json")):
        ...
        backup = safe_path(root, f"migration-backups/{identifier}/0")
        inventory = journal["inputs"][0]
        if not any(Path(relative).match("projects/*/hosts/*/layout.json") for relative in inventory):
            if not (backup / "schema.json").exists() or read_object(backup / "schema.json").get("schema_version", 17) > 15:
                continue
        context = MigrationContext(root, "shared", "", backup)
        flat_projects: set[str] = set()
        for relative, _payload in pristine_hints(context):
            if inventory.get(relative) is None or file_digest(safe_path(backup, relative)) != inventory[relative]:
                raise MigrationError("migration_failed", "历史布局备份摘要不匹配", backup / relative)
