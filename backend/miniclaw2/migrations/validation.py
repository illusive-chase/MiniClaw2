...
def validate(root: Path, *, external: bool = False) -> None:
    ...
    for relative in files(root, external=external):
        ...
        try:
            if path.name == "project.json" and len(parts) == 3 and parts[0] == "projects":
            ...
            elif path.name == "node.json" and parts[0] == "projects":
                if len(parts) != 7 or parts[2] != "hosts" or parts[4] != "nodes":
                    raise ValueError("节点不在 host 分区内")
            ...
            elif path.name in {"local.json", "layout.json", "host.json", "head.json", "git_aliases.json"} and parts[0] == "projects":
                payload = read_object(path)
            ...
            elif path.name in {"events.jsonl", "gates.jsonl"} and parts[0] == "projects":
