...
def test_owner_union_filters_stale_entries_without_pruning_shards(
    tmp_path: Path, only_missing: bool,
) -> None:
    ...
    local_positions = {native.id: local_position, foreign.id: remote_position, "gone": local_position}
    atomic_json(local_file, {"schema_version": 1, "nodes": local_positions})
    ...
    assert store.update_node_positions(project.id, {}, [], only_missing=only_missing) == positions
    assert json.loads(local_file.read_text())["nodes"] == local_positions
    ...
    store.update_node_positions(project.id, {}, [], only_missing=only_missing)
    assert json.loads(local_file.read_text())["nodes"] == local_positions
    native.planspace_id = None
    store.update_node(native)
    assert store.read_node_positions(project.id)[native.id].model_dump() == local_position
    store.delete_node(project.id, native.id)
    assert native.id not in store.read_node_positions(project.id)
    remaining = store.update_node_positions(project.id, {}, [], only_missing=only_missing)
    assert {key: value.model_dump() for key, value in remaining.items()} == {foreign.id: remote_position}
    assert json.loads(local_file.read_text())["nodes"] == local_positions
    assert peer_file.read_bytes() == original_peer
